...
output_file = os.path.join(folder_path, 'postVideosIndexes.ts')
...
# Filtra i file che sono immagini (es. con estensioni comuni come .mp4, .avi .mkv
image_files = [f for f in file_list if f.lower().endswith(('.mp4', '.avi', '.mkv', '.gif'))]
...
try:
    with open(output_file, 'w') as f:
        ...
        for img in image_files:
            f.write(f"    '/storedcatvideos/video/{img}',\n")
