...
export const FakeTextPostFactory = ():string => {
  ...
  for (let index = 0; index < numberOfWords; index++) {
    returnString += fakePostSortWords[Math.floor(Math.random() * fakePostSortWords.length)] + ' '
  }   
...
export const FakePostFactory = async (time: Date, author:UserDetails, num:number): Promise<Post> => {
  ...
  let fakePostText = FakeTextPostFactory()
