...
export const MakeFakeAccountNoPosts = async (accounts: CasualUser[]): Promise<CasualUser> => {
    const fakeCredential = await CreateFakeUser();
    const fakeAccountsPosts: Post[] = []
    const friendList = []
    const randomFriendNumber = Math.floor(Math.random() * accounts.length )+1
    for (let index = 0; index < randomFriendNumber; index++) {
